...
const kafka = new Kafka({
  ...
  brokers: ['localhost:9092'], // Asegúrate de que Kafka esté corriendo en esta dirección
});
...
(async () => {
  ...
  const browser = await puppeteer.launch({
    ...
    args: ['--no-sandbox', '--disable-setuid-sandbox'], // Añadido para evitar problemas con el sandbox
  });
  ...
  page.on('response', async (response) => {
    ...
    if (url.includes('web-events')) {
      try {
        ...
        console.log('Datos de tráfico:', jsonResponse);
